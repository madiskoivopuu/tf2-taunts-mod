...
public Plugin myinfo = 
{
	...
	author = "fakuivan, small tweaks by Sparkly Cat",
...
public void OnAllPluginsLoaded()
{
	...
	if (gi_initialization == InitializationStatus_Success)
	{
		...
		hTauntParticle[0] = RegClientCookie("tf2_taunts_particle", "", CookieAccess_Private);
	}
}
...
public Action Command_ForceSelfToTaunt(int i_client, int i_args)
{
	...
	if (CheckAndReplyCacheNotLoaded(i_client))return Plugin_Handled;
#endif //}
	if(Steam_InGroup(i_client) == 0)
	{
		PrintToChat(i_client, "[SM] You must be in our steam group to use this command. Type !group to join our group.");
		return Plugin_Handled;
	}
...
bool MenuMaker_TauntsMenu(int i_client, MenuHandler f_handler, any a_data = 0)
{
	...
	AddMenuItem(h_menu, "10000", "Unusual Taunt Effects");
...
public int MenuHandler_TauntsSelfMenu(Menu h_menu, MenuAction i_action, int i_param1, int i_param2)
{
	...
	{
		char menuitem[9];
		GetMenuItem(h_menu, i_param2, menuitem, sizeof(menuitem));
		int sItem = StringToInt(menuitem);
		if(sItem == 10000)
		{
			Menu MenuH = CreateMenu(UnuMenuCallback);
			SetMenuTitle(MenuH, "Taunts menu - Unusual Effects");
			AddMenuItem(MenuH, "0", "Remove Taunt Unusual Effect");
			AddMenuItem(MenuH, "1", "------------------------", ITEMDRAW_DISABLED);
			AddMenuItem(MenuH, "utaunt_firework_teamcolor_red", "Showstopper (RED)");
			AddMenuItem(MenuH, "utaunt_firework_teamcolor_blue", "Showstopper (BLU)");
			AddMenuItem(MenuH, "utaunt_beams_yellow", "Holy Grail");
			AddMenuItem(MenuH, "utaunt_disco_party", "'72");
			AddMenuItem(MenuH, "utaunt_hearts_glow_parent", "Fountain of Delight");
			AddMenuItem(MenuH, "utaunt_meteor_parent", "Screaming Tiger");
			AddMenuItem(MenuH, "utaunt_cash_confetti", "Skill Gotten Gains");
			AddMenuItem(MenuH, "utaunt_tornado_parent_black", "Midnight Whirlwind");
			AddMenuItem(MenuH, "utaunt_tornado_parent_white", "Silver Cyclone");
			AddMenuItem(MenuH, "utaunt_lightning_parent", "Mega Strike");
			AddMenuItem(MenuH, "utaunt_souls_green_parent", "Haunted Phantasm");
			AddMenuItem(MenuH, "utaunt_souls_purple_parent", "Ghastly Ghosts");
			AddMenuItem(MenuH, "utaunt_hellpit_parent", "Hellish Inferno");
			AddMenuItem(MenuH, "utaunt_hellswirl", "Spectral Swirl");
			AddMenuItem(MenuH, "utaunt_headless", "Infernal Flames");
			AddMenuItem(MenuH, "utaunt_merasmus", "Infernal Smoke");
			AddMenuItem(MenuH, "utaunt_bubbles_glow_green_parent", "Acidic Bubbles of Envy");
			AddMenuItem(MenuH, "utaunt_bubbles_glow_orange_parent", "Flammable Bubbles of Attraction");
			AddMenuItem(MenuH, "utaunt_bubbles_glow_purple_parent", "Poisonous Bubbles of Regret");
			AddMenuItem(MenuH, "utaunt_firework_dragon_parent", "Roaring Rockets");
			AddMenuItem(MenuH, "utaunt_smoke_moon_parent", "Spooky Night");
			AddMenuItem(MenuH, "utaunt_smoke_moon_green_parent", "Ominous Night");
			DisplayMenu(MenuH, i_param1, 60);
		}
		else{
...
public int UnuMenuCallback(Menu MenuH, MenuAction i_action, int i_param1, int i_param2)
{
	if(i_action == MenuAction_Select)
	{
		char item[72];
		GetMenuItem(MenuH, i_param2, item, sizeof(item));
		SetClientCookie(i_param1, hTauntParticle[0], item);
		PrintToChat(i_param1, "[Taunts] Taunt Unusual Effect set.");
	}
}
